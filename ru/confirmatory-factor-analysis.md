...
> **Порядковые данные?** Если индикаторы — пункты шкалы Ликерта (например, 1–5 или 1–7), используйте WLSMV (робастный, с поправкой среднего и дисперсии вариант DWLS). Стандартный ML рассматривает порядковые ответы как непрерывные, что может смещать оценки и завышать хи-квадрат. WLSMV корректно моделирует скрытые пороги и даёт робастные стандартные ошибки. lavaan автоматически переключает ML / DWLS на WLSMV, если хотя бы один индикатор помечен как порядковый — в [сводке модели](#результаты-для-одной-группы) показывается, какой метод был фактически использован и какой запрашивался, если они различаются. При обнаружении порядковых переменных диагностика в разделе [Проверка данных](#проверка-данных) порекомендует метод с пороговой моделью.
...
При включении вычисляются скорректированные на смещение ускоренные доверительные интервалы (BCa). Число репликаций задаётся в глобальной [настройке бутстрэпа](./settings.md#итерации-бутстрэпа).
...
В правой колонке расположен раскрывающийся раздел с **синтаксисом lavaan**. Это *не* односторонний предпросмотр — всё, что вы вводите в этом поле, переносится обратно в матрицу, а всё, что вы делаете в матрице, отражается в тексте. Кнопок «Применить» и «Отмена» нет — матрица обновляется автоматически по мере ввода. Парсер поддерживает `=~` для нагрузок, `~~` для ковариаций, фиксированные значения вида `1*x1`, метки равенства, имена в обратных кавычках и продолжения строк, заканчивающиеся на `+`. Кнопка **Копировать** копирует текущий текст в буфер обмена.
...
Результаты выводятся в карточке **Конфирматорный факторный анализ**. В верхней части — сводка модели: факторы, количество индикаторов, метод оценки, степени свободы, объём выборки, число свободных параметров и соотношение N/параметры. Если lavaan автоматически переключил метод оценки (например, ML → WLSMV из-за порядкового индикатора), в сводке отображается фактически использованный метод, а в скобках — запрошенный, чтобы замена не оставалась незаметной. При FIML отображаемый объём выборки — полное N (lavaan оценивает на каждом наблюдении), а не только полные случаи.
...
На путях отображаются значения нагрузок. Легенда в нижней части поясняет визуальное кодирование.
...
> Панель позволяет легко сопоставлять стандартизованные и нестандартизованные оценки бок о бок без повторного запуска. По умолчанию виден столбец, соответствующий выбору в опции [Стандартизация](#стандартизация) до запуска.
...
- **Станд. (только латентные)** — стандартизация только по дисперсии латентных переменных
- **Станд. (полностью)** — стандартизация по дисперсии как латентных, так и наблюдаемых переменных. Для нагрузок интерпретируется как корреляция между индикатором и фактором.
...
Каждый анализ выводит соответствующий R-код в [консоль R](./r-console.md) — его можно просматривать, копировать и повторно запускать. КФА использует пакет R `lavaan`; метрики надёжности — `semTools`. Ссылки на использованные пакеты R автоматически появляются в начале раздела вывода. Предпросмотр синтаксиса lavaan также позволяет экспортировать спецификацию модели напрямую. Когда включены бутстрэп-СО (в том числе для конфигуральной / метрической / скалярной / строгой моделей при проверке инвариантности), повторные выборки lavaan инициализируются настройкой [**Зерно бутстрэпа**](./settings.md#зерно-бутстрэпа) — задайте её, чтобы бутстрэп-СО воспроизводились между запусками.
