...
Каждый анализ выводит соответствующий R-код в [консоль R](./r-console.md) — его можно просматривать, копировать и повторно запускать. МСУ использует пакет R `lavaan`; метрики надёжности и дискриминантной валидности — `semTools`. Ссылки на пакеты автоматически появляются в начале карточки вывода. Поле синтаксиса lavaan также позволяет экспортировать спецификацию модели напрямую. Когда включены бутстрэп-СО, повторные выборки lavaan инициализируются настройкой [**Зерно бутстрэпа**](./settings.md#зерно-бутстрэпа) — задайте её, чтобы бутстрэп-СО и ДИ непрямых эффектов воспроизводились между запусками.
