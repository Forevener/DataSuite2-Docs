---
title: Корреляционный анализ в DataSuite 2
description: Корреляции Пирсона, Спирмена, Кендалла, полихорические, точечно-бисериальные, V Крамера и другие методы с выводом в виде матрицы, длинной таблицы и сетевой визуализации в DataSuite 2.
---
...
Установите флажок **Включить сетевую визуализацию**, чтобы сформировать сетевой граф в дополнение к таблице. Подробнее — в разделе [визуализация](#сетевая-визуализация-граф-корреляций).
...
## Сетевая визуализация (граф корреляций)

При включённом флажке визуализации формируется отдельная карточка результатов «Граф корреляций» с круговой сетевой диаграммой.

### Что показывает визуализация

- Переменные расположены по окружности в виде подписанных узлов
- Изогнутые линии (рёбра) соединяют пары со статистически значимыми корреляциями
...
- Переменные с более сильной корреляцией располагаются ближе друг к другу на окружности

### Взаимодействие с визуализацией

- **Наведение** на ребро — подсветить его
- **Масштабирование** — колесо мыши или кнопки +/−/сброс в правом верхнем углу
- **Изменение размера** — перетаскивание маркера в правом нижнем углу (минимум 400×400 пикселей)

### Экспорт

Под диаграммой доступны три кнопки экспорта:

- **SVG** — векторный формат, идеальный для публикаций и редактирования
