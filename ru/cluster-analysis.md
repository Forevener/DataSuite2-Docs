...
При включении диагностики стабильности, F-статистик или консенсусного оценивания добавляются соответствующие столбцы: **Стабильность** (Жаккар), **Средн. F строк** / **Средн. F столбцов** вместе с **Знач. строк (%)** / **Знач. столбцов (%)**, а также **Консенсус** (Жаккар).
...
Пропущенные значения обрабатываются построчным удалением — используются только полные наблюдения. Если число полных наблюдений отличается от общего, в выводе указываются оба. Воспроизводимостью управляет настройка [**Зерно воспроизводимости**](./settings.md#зерно-воспроизводимости) (по умолчанию 42); бутстрэп стабильности в бикластеризации использует отдельную настройку [**Зерно бутстрэпа**](./settings.md#зерно-бутстрэпа).
...
Каждый анализ выводит соответствующий R-код в [консоль R](./r-console.md) — его можно просматривать, копировать и повторно запускать. Кластерный анализ использует базовые функции R (`kmeans`, `hclust`) и пакет `cluster` (для PAM и силуэтного анализа); бикластеризация — пакеты `biclust` и `fabia`. Ссылки на использованные пакеты R автоматически появляются в начале раздела вывода. Шаги кластеризации и бикластеризации инициализируются настройкой [**Зерно воспроизводимости**](./settings.md#зерно-воспроизводимости) (по умолчанию 42); бутстрэп стабильности в бикластеризации инициализируется независимо настройкой [**Зерно бутстрэпа**](./settings.md#зерно-бутстрэпа), чтобы их можно было варьировать раздельно.
