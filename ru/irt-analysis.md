...
Каждый анализ печатает соответствующий R-код в [консоль R](./r-console.md). IRT-анализ использует R-пакет `mirt` для подбора модели, вращений, параметров пунктов и испытуемых, статистик подгонки, DIF и перевода баллов. Предварительный анализ дополнительно использует `mokken` для шкалируемости, монотонности, IIO, локальной независимости по rest-score и непараметрической надёжности. Цитирования R-пакетов автоматически появляются в начале вывода. Шаг подвыборки для RIR (resampled individual reliability) инициализируется настройкой [**Зерно бутстрэпа**](./settings.md#зерно-бутстрэпа) — задайте его, чтобы значения RIR воспроизводились между запусками.
