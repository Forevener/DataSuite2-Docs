...
- **Дисперсия, %** — доля общей дисперсии, объяснённая этим фактором
- **Кумулятивная, %** — нарастающий итог. Для АГК приближается к 100% по мере добавления факторов; для РФА выходит на «потолок» общей дисперсии (сумма коммунальностей ÷ число переменных) и не достигает 100%
...
Каждый анализ выводит соответствующий R-код в [консоль R](./r-console.md) — его можно просматривать, копировать и повторно запускать. Факторный анализ использует пакет R `psych`; ссылки на использованные пакеты R автоматически появляются в начале раздела вывода. Параллельный анализ (когда включён) строит Монте-Карло нулевые распределения, инициализированные настройкой [**Зерно воспроизводимости**](./settings.md#зерно-воспроизводимости) — оставьте пустым для свежих нулевых распределений при каждом запуске или задайте целое значение для стабильных сравнений.
