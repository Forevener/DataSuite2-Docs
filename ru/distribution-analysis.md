...
При наведении на любой столбец отображается количество наблюдений и диапазон значений. Если данные целочисленные и содержат не более 50 уникальных значений, интервалы автоматически выравниваются по отдельным целым числам — один столбец на каждое наблюдённое значение, а деления оси проставлены только в этих точках. Это работает как для плотных целых (например, 0–20), так и для разреженных (например, в основном 0–5 с несколькими далёкими выбросами); разреженные данные дают широкие крайние столбцы, наглядно отражающие разрыв. В разреженном целочисленном случае кривые плотности и нормального распределения скрываются (с соответствующей пометкой под графиком), потому что ширины интервалов различаются на порядки, и пересчёт через среднюю ширину интервала дал бы визуально бессмысленные кривые.
