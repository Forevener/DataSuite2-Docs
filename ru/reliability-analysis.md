...
Каждый анализ выводит соответствующий R-код в [консоль R](./r-console.md) — его можно просматривать, копировать и повторно запускать. Внутренняя согласованность использует пакет R `psych`. Воспроизводимость дополнительно использует `irr` (каппа, α Криппендорфа, W Кендалла), `lme4` (зависимость ICC) и `tidyr` (преобразование данных). Ссылки на использованные пакеты R автоматически появляются в начале раздела вывода.
