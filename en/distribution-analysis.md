...
description: Frequency tables, normality tests, histograms, box plots, Q-Q plots, violin plots, ECDF plots, bar charts, and pie charts in DataSuite 2.
---
...
One table is produced per variable. To see the same counts as a figure, run the variable through [distribution plots](#distribution-plots) with **Bar chart** or **Pie chart** checked.
...
> **Sort options when compressing into ranges:** count-based sorts don't apply to tables that were actually compressed into ranges – sorting ranges by count would scramble the contiguous range axis – so those tables fall back to value order, with a note below the table saying so. Tables that aren't compressed (categorical variables, or numeric variables under the **Maximum categories** threshold) keep the requested count sort.
...
Compression is off by default, so a numeric variable is tallied value by value until you turn it on. When that produces more rows than **Maximum categories** would have allowed, a note below the table reports the distinct-value count and points at the option. The table itself is left in full – quietly binning or truncating a result you asked for would be worse than a long table.

Range labels read as `170..178` — the inclusive value range each bin covers, with adjacent bins stepping to the next value (`179..187`) instead of repeating a shared edge. When your data is finer-grained than your global [number-precision setting](./settings.md#precision-settings) can display (e.g. three-decimal values at two-decimal precision), labels fall back to a `170.00..<180.00` form, where `..<` reads as "up to, but not including" the upper edge.
...
| **D'Agostino-Pearson** | K² | 8 (max 46340) | Tests skewness and kurtosis jointly. Reliable from about n ≥ 20 – see the recommendation note below. |
...
> **Which test to pick?** Shapiro-Wilk is the best default – it has the highest statistical power in most situations. Shapiro-Francia is a strong alternative once n ≥ 50 and has the appealing property of being directly tied to the Q-Q plot (high W' = points close to the line). If you want a second opinion, add Anderson-Darling. If your sample is very large (n > 5000), consider D'Agostino-Pearson or Jarque-Bera, as Shapiro-Wilk can't even run at that size. D'Agostino-Pearson itself tops out at n = 46340 (a limit of its R implementation); beyond that, Jarque-Bera has no upper bound.
...
A single table with one row per variable. For each selected test, two columns appear – the test statistic and the p-value – under a spanned header naming the test, so a table of seven tests doesn't read as seven identical **p** columns (the symbols alone won't separate *W* for Shapiro-Wilk from *W'* for Shapiro-Francia and *W²* for Cramér-von Mises). Significance formatting follows your [settings](./settings.md#p-value-settings).
...
- **Multiple tests selected** – shows an agreement summary like "4/6 tests: evidence against normality" with a tri-state colored verdict: green when all valid tests fail to reject, red when all of them reject, and amber for any mix in between. Tests that were not computed (e.g. due to sample size limits) are reported separately and excluded from the count.
...
1. Select one or more variables – numeric for most plot types, categorical for the bar and pie charts
...
Each plot type draws only the variable types it can serve: the histogram, box plot, Q-Q plot, violin plot, and ECDF plot need a numeric scale, the bar chart takes categorical variables, and the pie chart takes either. A mixed selection isn't an error – every plot type is drawn against the variables it serves, and a note above the plots names the ones it passed over and why ("Excluded from Histogram, Box plot (not numerical): …"). If nothing in your selection can be drawn by any of the chosen plot types, a warning is shown and no card is produced; if the variables have no valid data at all, a note is shown in place of an empty card.
...
- **Q-Q plot** – supported for all five reference distributions. Each variable is standardized by its own reference line – the qqline through its sample and theoretical quartiles, or the through-the-origin line with slope = mean for the Exponential, and after a log transform for Lognormal – so different scales sit on common axes and the reference becomes the line *y = x* for every family. The detrended toggle and the confidence band still work – the band is drawn per group around *y = x* in standardized space.
- **Histogram, Bar chart, Pie chart** – always skipped in overlay mode: each shows one variable's whole distribution, which is exactly what a shared axis can't hold. Generate them per-variable by turning overlay off.
...
If you select **only** plot types that overlay mode skips (e.g. just Histogram, or only Bar chart and Pie chart), no result card is produced and a warning is shown – turn overlay off or pick a compatible plot type to proceed.

Variables with no valid numerical data are excluded from the overlay and listed in an info note above the plots. Violin plots and the Q-Q overlay additionally drop variables with fewer than 2 valid observations (no spread to summarize) and zero-variance variables (a constant carries no distributional information); each case appears in its own per-plot note below the chart ("Excluded (n < 2): …" and "Excluded zero-variance: …"). Box plots keep such variables and draw them as a degenerate flat box – their five-number summary is still defined, matching R's behavior.
...
Hover over any bar to see the count and value. When the data is integer-valued with at most 50 distinct values *and* the bin method is left on Auto, the histogram switches to a discrete layout – one bar per value rather than arbitrary bins. Picking Sturges, Scott, or Freedman-Diaconis explicitly overrides that: the rule you chose is applied to the integer data, and a note below the chart says so and points back at Auto for one bin per value. The discrete layout has two variants, chosen automatically:
...
- **Median notch** – adds a notch around the median spanning its confidence interval, computed distribution-free from the order statistics (Hettmansperger–Sheather interpolation) at your global [confidence level](./settings.md#confidence-level). The notch is asymmetric when the data are skewed, and is omitted for groups too small to support the interval at that level. When a group's median CI is wider than its box, the notch is clamped to the box hinges and a note below the plot lists the affected groups. If the notches of two box plots don't overlap, their medians are likely significantly different. To get the same interval as a number, select **CI for median** in [Descriptive statistics](./descriptive-statistics.md#confidence-intervals) and set its **Median CI method** to **Interpolated (Hettmansperger-Sheather)** – the default exact method is conservative and gives a slightly wider interval than the notch.
- **Data points** – displays individual observations alongside the box, giving you the full picture rather than just the summary. Outlier observations are always included among the jittered points; **Show outliers** controls only whether they get the diamond-marker emphasis (diamonds sit among the jittered points rather than stacked on the centre line, so heavy tails stay readable and outliers remain distinguishable from ordinary points)
...
- **Detrended (residuals vs reference)** – subtracts the reference line from each point's y-coordinate, so the reference becomes a horizontal line at zero. The y-axis relabels to residuals to match. Small deviations become much easier to see than in the standard "diagonal" view.
...
The empirical cumulative distribution function shows, for each value, what percentage of the data falls at or below it. It rises from 0% to 100% as a step function.
...
The frequency table's companion figure: one bar per category, tallest first. Categorical variables only – a bar chart of discrete numbers would be the histogram's discrete layout bar for bar, so numeric variables get a histogram (or a pie chart) instead.

- Bars are ordered by count, highest first, with ties broken alphabetically
- Past 25 categories the rare tail is folded into a single **Other** bar, and a note below the chart says how many categories it stands for
- Category names are tilted automatically once they start to crowd each other

Hover over a bar to see its count and category name.

### Pie chart

Shows each slice's share of the whole. Unlike every other plot here, it serves all variable types:

- **Categorical** – one slice per category, largest first. Past 10 categories the rare tail is folded into a single **Other** slice, with a note below the chart saying how many categories it covers
- **Integer-valued numeric** (at most 50 distinct values, with **Bin calculation method** left on Auto) – one slice per value, matching the histogram's discrete layout bar for bar
- **Continuous numeric** – one slice per bin, cut with the same **Bin calculation method** the histogram uses, so a pie and a histogram of the same variable agree slice for bar. Empty bins are dropped rather than drawn as zero-width wedges

Slices wide enough to hold a label show their percentage inside the wedge; the legend on the right names every slice with its count. Hover over a slice for its label, count, and percentage.

> **Pie or bar?** A pie answers "what share of the whole?" for a handful of categories. Comparing categories *to each other* is easier on a bar chart – length is easier to judge than angle – and past a few categories the bar chart or the frequency table reads better than any pie.
...
Normality tests print the underlying R code to the [R console](./r-console.md) – you can inspect, copy, or re-run the exact commands. The module uses base R (`shapiro.test`) plus the `nortest` package (Anderson-Darling, Lilliefors, Cramér-von Mises, Shapiro-Francia), `moments` (Jarque-Bera and D'Agostino-Pearson), depending on which tests you select. Citations appear automatically at the top of the output section, and cover both the software and the methods: every normality test you run cites its own source, and every plot cites what it draws – the binning rule the histogram used, the kernel density estimate behind a density curve or a violin, Tukey's fences and the median-CI notch on a box plot, the Q-Q plot, whichever ECDF band you picked, and the bar or pie chart. Frequency tables and distribution plots are computed in JavaScript and do not produce R code.
