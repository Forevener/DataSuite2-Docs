...
| Paired t-test | Cohen's dz, Hedges' gz, Cohen's d_av |
...
| Sign test | sign-test r (2p₊ − 1) |
| One-way ANOVA, MANOVA | η², ω², ε² |
| Welch's ANOVA, Brown-Forsythe ANOVA, Yuen's test, trimmed-means ANOVA | explanatory measure ξ |
| Kruskal-Wallis | ε², η²_H |
| Friedman | Kendall's W |
| Repeated measures ANOVA | partial η², η², partial ω² |
...
The tests whose statistic is already a bounded magnitude in its own right offer no separate measure, so **Include effect sizes** has nothing to add for them: **Kolmogorov-Smirnov** and **Anderson-Darling**, **Bartlett's test** and **Fligner-Killeen**, and the two multivariate tests that estimate no pooled error term – [**Johansen's heteroscedastic MANOVA**](#independent-samples--numeric) and the [**energy test of equal distributions**](#independent-samples--numeric).
...
>
> **The trend tests' companion measures.** J and L are on scales of their own, so each ordered-alternative test reports the measure conventionally paired with it. **Kendall's τ-b** (Jonckheere-Terpstra) restates the same monotone association on a −1 to +1 correlation scale, tie-corrected – J is a monotone function of it, so the two always agree in direction, and because τ-b is computed from the data it is available on the permutation branch too. **Page's r** = Z/√N (Page's trend test) puts the trend on the module's Wilcoxon-r convention, with N the total number of observations across the complete blocks. Both use **bootstrap percentile CIs** when effect-size intervals are enabled – τ-b resampling within group so the design's group sizes stay fixed, Page's r resampling whole blocks (subjects) – marked with a superscript "b" like the other resampled intervals.
...
Check **Include descriptive discriminant analysis** to name the combination of dependent variables the groups separate on. The option appears for **MANOVA** and **MANCOVA** only – they are the two tests whose omnibus has a canonical decomposition to describe. It answers the question the multivariate test asks and the per-variable follow-ups can't, and it is added *beside* those follow-ups rather than in place of them; see [discriminant analysis results](#descriptive-discriminant-analysis) for the four tables it produces.
...
The dropdown is hidden in a **purely within-subjects** design, where the [Cousineau-Morey interval](#mean-and-error-bar-plot) is the only meaningful bar and has no SD or SE analogue. In a mixed design it stays available, but any cell drawn with a within-subject interval still overrides it.
...
- Expected frequencies (**Exp:**) and Pearson residuals (**Res:**) beneath each observed count (chi-square test; a note under the table explains the two prefixes)
- Row and column totals

> **Pearson residuals show where the association lives.** Each cell's residual is (observed − expected) / √expected – how far the cell sits above (+) or below (−) its expectation, in standardized units. Cells beyond roughly ±2 are the ones driving a significant χ²: the test reports the overall verdict, the residuals show which cells carry it.
...
1. **Effects table** – each effect labeled "(between)", "(within)", or "(covariate)". With 3+ conditions, additional columns show both Greenhouse-Geisser (GG) and Huynh-Feldt (HF) epsilon values, the corrected degrees of freedom each implies, and their corrected p-values. When a complete-subjects count is available (repeated measures factorial ANOVA), the table header reports **N = {n} complete subjects** – the subjects with complete data behind the omnibus F, so any listwise drop stays visible.
...
5. **Descriptive statistics** – per-DV, per-group summaries
...
A spaghetti plot connecting each subject's values across conditions, with a bold mean line overlay. Aligns by subject ID when available, falls back to row correspondence. With two or more condition factors (repeated-measures factorial designs), each x-position is one cell of their full crossing, so every subject's line spans all factor combinations. Option: show mean line. The number of subjects actually drawn is captioned below the plot, and any dropped along the way – missing a value in some condition, or carrying duplicate condition rows – are reported with their count in a toast.
